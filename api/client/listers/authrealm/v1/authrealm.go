...
type AuthRealmLister interface {
	...
	List(selector labels.Selector) (ret []*v1.AuthRealm, err error)
...
// List lists all AuthRealms in the indexer.
func (s *authRealmLister) List(selector labels.Selector) (ret []*v1.AuthRealm, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.AuthRealm))
	})
	return ret, err
}
...
type AuthRealmNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.AuthRealm, error)
...
// List lists all AuthRealms in the indexer for a given namespace.
func (s authRealmNamespaceLister) List(selector labels.Selector) (ret []*v1.AuthRealm, err error) {
...
// Get retrieves the AuthRealm from the indexer for a given namespace and name.
func (s authRealmNamespaceLister) Get(name string) (*v1.AuthRealm, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("authrealm"), name)
	}
	return obj.(*v1.AuthRealm), nil
}
