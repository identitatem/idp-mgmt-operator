...
var _ = Describe("Process Strategy backplane: ", func() {
	...
	It("Process the creation of a placementDecision", func() {
		...
		By(fmt.Sprintf("Checking client secret %s", AuthRealmName), func() {
			err := k8sClient.Get(context.TODO(), client.ObjectKey{Name: helpers.ClientSecretName(authRealm, ClusterName), Namespace: ClusterName}, clientSecret)
			Expect(err).To(BeNil())
		})
		...
		By(fmt.Sprintf("Checking ClusterOAuth %s", dexClientName), func() {
			...
			err := k8sClient.Get(context.TODO(), client.ObjectKey{Name: helpers.ClusterOAuthName(authRealm, ClusterName), Namespace: ClusterName}, clusterOAuth)
			Expect(err).To(BeNil())
